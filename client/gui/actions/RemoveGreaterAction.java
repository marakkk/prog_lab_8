...
public class RemoveGreaterAction extends Action{
    public RemoveGreaterAction(User user, ClientManager client, GuiManager guiManager) {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        // Default Values
        {
            ...
            personBirthdayField.setValue("2000-09-09");
            personHeightField.setValue("120");
            personWeightField.setValue("50");
        }
        // Group Layout
        {
        ...
        if(result == OK_OPTION){
            ...
            try {
                response = client.processRequestToServer(new ClientRequest("remove_greater", "",ticket, user,  GuiManager.getLocale()));
            } catch (ConnectionErrorException | NotDeclaredLimitsException ex) {
                throw new RuntimeException(ex);
            }
            if(response.getResponseCode() == ResponseCode.OK) JOptionPane.showMessageDialog(null, resourceBundle.getString("ObjectDeleted"), resourceBundle.getString("Ok"), JOptionPane.PLAIN_MESSAGE);
